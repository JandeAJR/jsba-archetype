...
@Mapper(componentModel = "spring", uses = { ApiRoleMapper.class })
public interface ApiUserMapper {
	// From Domain To DTO (User -> UserResponse)
 	UserResponse toDtoFromDomain(User user); 	
 	
 	// From Output UseCase To DTO (UserOutput -> UserResponse)
 	UserResponse toDtoFromApplicationOutput(UserOutput userOutput);
 	List<UserResponse> toDtoFromApplicationOutputList(List<UserOutput> outputs);
 	
 	// Método default para lidar com a paginação.
    default Page<UserResponse> toResponsePage(Page<UserOutput> page) {
        if (page == null) {
            return null;
        }
