...
/**
 * Classe: ListUsersByNameQuerySpec <br>
 * Pacote: ${package}.security.jwt.core.application.query.spec
 * 
 * <p>Classe responável por implementar a query listar usuário pelo nome, 
...
public class ListUsersByNameQuerySpec {
	...
	public Page<UserOutput> list(UserFilter filter, Pageable pageable) {
		Specification<UserEntity> spec = Specification
				.where(QuerySpecification.<UserEntity>criteriaByString(
						"name",              // Nome exato do atributo na Entity JPA
						filter.name(),       // Filtro informado na query string da url da requisição http
						filter.operator()));
